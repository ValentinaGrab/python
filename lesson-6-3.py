...
with open('users.csv', 'r', encoding='utf-8') as file_users:
    with open('hobby.csv', 'r', encoding='utf-8') as file_hobbies:
        while True:
            fio = file_users.readline().rstrip().replace(',', ' ')
            # если ФИО больше нет, то выход из цикла
            if not fio:
                break
            hobby = file_hobbies.readline().rstrip()
            if not hobby:
                new_dict[fio] = None
            else:
            # Добавление пары ключ-значение в справочник
                new_dict[fio] = hobby
print(new_dict)
